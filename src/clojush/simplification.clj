    ...
    (if (>= step steps)
      ...
      (let [new-program (if (< (lrand-int 5) 4)
            ...
            new-errors (if (not lazy-automatic-simplification)
                         (:errors (error-function {:program new-program :library (:library ind)}))
                         (loop [remaining-inputs training-cases
                           ...
                           (if (empty? remaining-inputs)
                             ...
                             (let [new-error (:errors (error-function {:program new-program :library (:library ind)} (take 1 remaining-inputs)))
