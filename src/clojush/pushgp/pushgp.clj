    ...
    (cond (= outcome :failure) (do (printf "\nFAILURE\n")
          ...
          (= outcome :continue) (let [next-novelty-archive
                                  ...
                                  (swap! push-argmap assoc :seniors (repeatedly 50 (fn []
                                                                                     (get (auto-simplify-permute-plush (auto-simplify-plush (select (map #(deref %) pop-agents) @push-argmap) (:error-function @push-argmap) 25 0) (:error-function @push-argmap) (:training-cases @push-argmap) 25 false 0)
                                                                                                :genome))))
