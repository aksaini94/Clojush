...
an empty stack. This is a utility, not for use as an instruction in Push programs. "
  ...
  (let [stack (type state)]
    (if (empty? stack)
      :no-stack-item
      (if (and (:simplification-by-permutation state) (map? (first stack)) )
        (:value (first stack))
        (first stack))
      )))
...
NOT SAFE for invalid positions. "
  ...
  (let [stack (type state)]
    (if (empty? stack)
      :no-stack-item
      (if (and (:simplification-by-permutation state) (map? (nth stack position)) )
...
not for use as an instruction in Push programs. "
  [type state]
   (let [_ (if (and (:simplification-by-permutation state) (not (empty? (type state) ))  (map? (first (type state))) )
            (swap! global-id-arg assoc (:creation-time (first (type state))) (first (:auxiliary state))))]
